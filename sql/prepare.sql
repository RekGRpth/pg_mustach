...
SELECT 2, 'named template renders one jsonb value', mustach_render('{"people":[{"firstName":"Yehuda","lastName":"Katz"}]}', tplname := 'people');
SELECT 3, 'same named template, second jsonb value', mustach_render('{"people":[{"firstName":"Carl","lastName":"Lerche"},{"firstName":"Alan","lastName":"Johnson"}]}', tplname := 'people');
SELECT mustach_prepare('{{b}}', 'people');
SELECT 4, 're-preparing an existing tplname replaces it rather than leaking the old one', mustach_render('{"a":"stale","b":"fresh"}', tplname := 'people');
...
SELECT 7, 'render on a forgotten tplname errors', mustach_render('{}', tplname := 'people');
\set ON_ERROR_STOP true
\set ON_ERROR_STOP false
SELECT 8, 'render on a tplname that was never prepared errors', mustach_render('{}', tplname := 'doesnotexist');
\set ON_ERROR_STOP true
...
RESET ROLE;
--
-- tplname has DEFAULT NULL on both mustach_render overloads, and a bare
...
--
SELECT 12, 'a bare positional 2nd argument is the file overload, not tplname', mustach_render('{"a":"b"}', '/tmp/pg_mustach_test_prepare_positional_footgun.txt');
...
\set ON_ERROR_STOP false
SELECT 16, 'render with no tplname errors once the default slot is empty', mustach_render('{}');
\set ON_ERROR_STOP true
ROLLBACK;
